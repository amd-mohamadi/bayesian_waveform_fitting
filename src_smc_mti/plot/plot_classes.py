...
from pyrocko.plot import hudson

# Use local SMCMTI implementations instead of MTfit package
from ..forward_model import station_angles
from ..moment_tesnor_conversion import (
    MT33_MT6,
    toa_vec,
    tk_uv,
    E_tk,
...
# Fallback get_extensions (MTfit plugin system) – no-op here
try:
    from ..utilities.extensions import get_extensions  # type: ignore
except Exception:
    def get_extensions(group=None, defaults=None):
        return [], defaults or {}

# Fallbacks for MTfit.sampling utilities
try:
    from MTfit.sampling import unique_columns, _6sphere_prior, ln_bayesian_evidence  # type: ignore
except Exception:
    def unique_columns(arr, return_counts=False, return_index=False):
        """Simple replacement: uniques along axis=1."""
        arr = np.asarray(arr)
        # reshape to (n_cols, n_rows) for np.unique axis=0
        arr_t = np.swapaxes(arr, 0, 1)
        uniq, idx, counts = np.unique(arr_t, axis=0, return_index=True, return_counts=True)
        uniq_cols = np.swapaxes(uniq, 0, 1)
        out = uniq_cols
        if return_counts and return_index:
    ...
    def _6sphere_prior(x):
        return 1.0

    def ln_bayesian_evidence(*args, **kwargs):
        return np.nan
...
class _FocalSpherePlot(_BasePlot):
    ...
    def plot_plane(self, strike, dip, c, radians=False, zorder=0, *args, **kwargs):
        ...
        """
        # Coerce strike/dip to scalar floats (MTfit logic is defined per plane)
        strike = np.asarray(strike, dtype=float).ravel()[0]
        dip = np.asarray(dip, dtype=float).ravel()[0]
        if not radians:
            strike = strike * np.pi / 180.0
            dip = dip * np.pi / 180.0
        v1 = toa_vec((strike + np.pi / 2.0), (np.pi / 2.0 - dip), radians=True)
        v2 = toa_vec(strike, np.pi / 2.0, radians=True)
    ...
    def _get_great_circle(self, v1, v2, az=(0, 2*np.pi), n=500):
        ...
        if isinstance(v1, tuple):
            ...
            v1 = np.matrix(v1).reshape(3, 1)
        if isinstance(v2, tuple):
            v2 = np.matrix([[v2[0]], [v2[1]], [v2[2]]])
        else:
            v2 = np.matrix(v2).reshape(3, 1)

        az = np.linspace(az[0], az[1], n)
        # Standard great-circle construction: if v1 and v2 are not parallel,
        # build an orthonormal basis spanning the plane that contains them.
        if np.abs(v1.T * v2)[0, 0] > 10**-10:
    ...
    def _stations(self, lim=np.sqrt(2), colors=('k', 'w', 'k'), marker=('v', 'o', '^')):
        ...
        if 'polarity' in self.stations.keys() and not np.all(np.array(self.stations['polarity']).flatten().astype(int) == np.array(self.stations['polarity']).flatten()):
    ...
    def _plot_stations(self, names, azimuth, takeoff_angle, polarity, marker=('v', 'o', '^'), colors=('k', 'w', 'k'), radians=False, lim=np.sqrt(2), zorder=0):
        ...
        elif not np.all(np.array(polarity).flatten().astype(int) == np.array(polarity).flatten()):
...
# ------------------------------------------------------------------
# Standalone plotting functions moved from synthetic_event.py
# ------------------------------------------------------------------
...
def kave(plungt, plungb, plungp):
    ...
    Parameters
    ----------
    plungt, plungb, plungp : float or array-like
        Plunge angles in degrees for T (tension), B (null), and P (pressure) axes.

    Returns
    -------
    x, y : float or arrays
        Coordinates on the Kaverina diagram.
    """
    zt = np.sin(np.deg2rad(plungt))
    zb = np.sin(np.deg2rad(plungb))
    zp = np.sin(np.deg2rad(plungp))
    L = 2 * np.sin(0.5 * np.arccos((zt + zb + zp) / np.sqrt(3)))
    N = np.sqrt(2 * ((zb - zp)**2 + (zb - zt)**2 + (zt - zp)**2))
    x = np.sqrt(3) * (L / N) * (zt - zp)
    y = (L / N) * (2 * zb - zp - zt)
    return x, y


def plot_kaverina_with_hdi(mt6_samples: np.ndarray, hdi_prob: float = 0.90,
                           min_threshold: float = 0.05,
                           output_path: str = "kaverina_hdi.png",
                           grid_size: int = 300,
                           n_contour_levels: int = 20,
                           cmap: str = 'Greys',
                           show_samples: bool = False,
                           mt6_median: np.ndarray | None = None,
                           mt6_mode: np.ndarray | None = None,
                           mt6_true: np.ndarray | None = None) -> None:
    """
    Plot Kaverina diagram with probability density contours from MT6 posterior samples.
    ...
    min_threshold : float, default 0.05
        Minimum normalized density threshold for plotting contours.
    output_path : str
        Path to save the figure.
    grid_size : int, default 300
        Resolution of the grid for contour plotting.
    n_contour_levels : int, default 50
        Number of contour levels.
    cmap : str, default 'YlOrRd'
        Matplotlib colormap name.
    """
    # Get eigenvalues and eigenvectors for all samples
    T_all, N_all, P_all, E_all = MT6_TNPE(mt6_samples)

    # Compute plunges for T, B, P axes (in degrees)
    # Plunge = 90 - inclination from vertical = arcsin(z_component)
    T_plunge = np.degrees(np.arcsin(np.abs(T_all[2, :])))  # z-component of T axis
    B_plunge = np.degrees(np.arcsin(np.abs(N_all[2, :])))  # z-component of N axis
    P_plunge = np.degrees(np.arcsin(np.abs(P_all[2, :])))  # z-component of P axis

    # Project onto Kaverina coordinates
    x_samples, y_samples = kave(T_plunge, B_plunge, P_plunge)
    ...
    # Draw Kaverina diagram frame
    # Outer triangle boundaries
    outer_xy_segments = []

    # T axis plunge line (bottom left to top)
    X = np.zeros(101)
    Y = np.zeros(101)
    for a in range(101):
        T = np.arcsin(np.sqrt(a/100.0)) / (np.pi/180)
        B = 0.0
        P = np.arcsin(np.sqrt(1-(a/100.0))) / (np.pi/180)
        X[a], Y[a] = kave(T, B, P)
    outer_xy_segments.append(np.column_stack([X, Y]))

    # Add tick marks and labels for T axis plunge
    tickx, ticky = kave(range(0, 91, 10), np.zeros(10), range(90, -1, -10))
    ax.plot(X, Y, color='black', linewidth=2)
    ax.scatter(tickx, ticky, marker=3, c='black', linewidths=2)
    for i in range(10):
        ax.text(tickx[i], ticky[i] - 0.04, i*10, fontsize=9, verticalalignment='top')
    ax.text(0, -0.75, 'T axis plunge', fontsize=14, horizontalalignment='center')

    # B axis plunge line (bottom right to top left)
    X = np.zeros(101)
    Y = np.zeros(101)
    for a in range(101):
        B = np.arcsin(np.sqrt(a/100.0)) / (np.pi/180)
        P = 0.0
        T = np.arcsin(np.sqrt(1-(a/100.0))) / (np.pi/180)
        X[a], Y[a] = kave(T, B, P)

    # Add tick marks and labels for B axis plunge
    tickx, ticky = kave(np.zeros(10), range(0, 91, 10), range(90, -1, -10))
    ax.plot(X, Y, color='black', linewidth=2)
    outer_xy_segments.append(np.column_stack([X, Y]))
    ax.scatter(tickx, ticky, marker=0, c='black', linewidths=2)
    for i in range(10):
        ax.text(tickx[i] - 0.04, ticky[i], i*10, fontsize=9, horizontalalignment='right')
    ax.text(-0.65, 0.2, 'B axis plunge', fontsize=14, horizontalalignment='center', rotation=60)

    # P axis plunge line (bottom)
    X = np.zeros(101)
    Y = np.zeros(101)
    for a in range(101):
        P = np.arcsin(np.sqrt(a/100.0)) / (np.pi/180)
        T = 0.0
        B = np.arcsin(np.sqrt(1-(a/100.0))) / (np.pi/180)
        X[a], Y[a] = kave(T, B, P)
    ax.plot(X, Y, color='black', linewidth=2)
    outer_xy_segments.append(np.column_stack([X, Y]))

    # Build polygon for valid Kaverina region (outer triangle)
    # The three segments connect as follows:
    # seg1: bottom-left (T=0, B=0, P=90) -> top (T=90, B=0, P=0)
    # seg2: top (T=90, B=0, P=0) -> bottom-right (T=0, B=90, P=0)
    # seg3: bottom-right (T=0, B=90, P=0) -> bottom-left (T=0, B=0, P=90)
    # They should connect naturally in this order
    kaverina_boundary = np.vstack([
        outer_xy_segments[0],  # T axis line
        outer_xy_segments[1],  # B axis line
        outer_xy_segments[2]   # P axis line
    ])
    kaverina_path = Path(kaverina_boundary, closed=True)

    # # Debug: print boundary vertices
    # print(f"\nKaverina boundary vertices:")
    # print(f"  First vertex (seg0[0]): {outer_xy_segments[0][0]}")
    # print(f"  seg0 -> seg1: {outer_xy_segments[0][-1]} -> {outer_xy_segments[1][0]}")
    # print(f"  seg1 -> seg2: {outer_xy_segments[1][-1]} -> {outer_xy_segments[2][0]}")
    # print(f"  Last vertex (seg2[-1]): {outer_xy_segments[2][-1]}")
    # print(f"  Total boundary points: {len(kaverina_boundary)}")

    # Interior division lines for faulting regimes
    # TF/SS boundary
    X = np.zeros(51)
    Y = np.zeros(51)
    for a in range(51):
        B = np.arcsin(np.sqrt((a/50.0)*0.14645)) / (np.pi/180)
        T = 67.5
        P = np.arcsin(np.sqrt((1-(a/50.0))*0.14645)) / (np.pi/180)
        X[a], Y[a] = kave(T, B, P)
    xf, yf = X[25], Y[25]
    ax.plot([0, xf], [0, yf], color='grey', linewidth=1)
    ax.plot(X[25:51], Y[25:51], color='grey', linewidth=1)
    ...
    for a in range(51):
        B = np.arcsin(np.sqrt((a/50.0)*0.14645)) / (np.pi/180)
        P = 67.5
        T = np.arcsin(np.sqrt((1-(a/50.0))*0.14645)) / (np.pi/180)
        X[a], Y[a] = kave(T, B, P)
    xf, yf = X[25], Y[25]
    ax.plot([0, xf], [0, yf], color='grey', linewidth=1)
    ax.plot(X[25:51], Y[25:51], color='grey', linewidth=1)
    ...
    Y = np.zeros(51)
    for a in range(51):
        T = np.arcsin(np.sqrt((a/50.0)*0.14645)) / (np.pi/180)
        B = 67.5
        P = np.arcsin(np.sqrt((1-(a/50.0))*0.14645)) / (np.pi/180)
        X[a], Y[a] = kave(T, B, P)
    ...
    # Vertical lines
    ax.plot([0, 0], [0.555221438, -0.605810893], color='grey', linewidth=1)
    ax.plot([0, 0.52481139], [0, 0.303], color='grey', linewidth=1)
    ax.plot([0, -0.52481139], [0, 0.303], color='grey', linewidth=1)

    # Labels
    ax.text(-0.9, -0.5, 'Normal', horizontalalignment='right', fontsize=11)
    ax.text(0.9, -0.5, 'Reverse', horizontalalignment='left', fontsize=11)
    ax.text(0, 1, 'Strike-slip', horizontalalignment='center', fontsize=11)

    # Plot density if sufficient variance
    if x_std > 1e-6 and y_std > 1e-6:
        # Create 2D KDE
        positions = np.vstack([x_samples, y_samples])
        kernel = gaussian_kde(positions)

        # Create grid
        # Use boundary extents so the KDE grid always covers the full triangle
        x_min, x_max = kaverina_boundary[:, 0].min(), kaverina_boundary[:, 0].max()
        y_min, y_max = kaverina_boundary[:, 1].min(), kaverina_boundary[:, 1].max()
        ...
        # Mask density outside the Kaverina triangle BEFORE plotting
        points_flat = np.column_stack([X_grid.ravel(), Y_grid.ravel()])
        inside = kaverina_path.contains_points(points_flat)
        Z_norm_flat = Z_normalized.ravel()
        Z_norm_flat[~inside] = np.nan
        Z_normalized_masked = Z_norm_flat.reshape(Z_normalized.shape)

        # print(f"Density range: min={np.nanmin(Z_normalized_masked):.6f}, max={np.nanmax(Z_normalized_masked):.6f}")
        # print(f"Points inside triangle: {inside.sum()} / {len(inside)} ({100*inside.sum()/len(inside):.1f}%)")

        # Hard clip the contour collection to the triangle to avoid bleed outside
        clip_patch = plt.Polygon(kaverina_boundary, closed=True,
                                 facecolor='none', edgecolor='none',
                                 transform=ax.transData)
        ax.add_patch(clip_patch)

        # ------------------------------------------------------------------
        # Compute and plot HDI contour lines (e.g. 90% region)
        # ------------------------------------------------------------------
        # `inside` is 1D over the flattened grid; use it on the flattened Z.
        z_valid = Z.ravel()[inside]
        ...
        if z_cumsum[-1] > 0:
            ...
            # Always compute 50% and requested hdi_prob (default 90%)
            hdi_levels = [0.50]
            if hdi_prob not in hdi_levels:
                hdi_levels.append(hdi_prob)

            # print("\nKaverina HDI thresholds:")
            hdi_thresholds = {}
            for level in sorted(hdi_levels):
                ...
                thr = z_sorted[idx]
                thr_norm = thr / Z.max()
                # print(f"  {int(level*100)}% HDI: threshold={thr:.6e} (normalized={thr_norm:.6f})")
                hdi_thresholds[level] = thr_norm

            # ------------------------------------------------------------------
            # Background color: only show density inside the 90% HDI region
            # ------------------------------------------------------------------
            if hdi_prob in hdi_thresholds:
                thr90 = hdi_thresholds[hdi_prob]
                Z_bg = Z_normalized_masked.copy()
                Z_bg[Z_bg < thr90] = np.nan
                contour_levels_array = np.linspace(thr90, 1.0, n_contour_levels)
                ...
                contourf_plot.set_clip_path(clip_patch)

            # ------------------------------------------------------------------
            # HDI contour lines: 50% (optional) and 90%
            # ------------------------------------------------------------------
            for level, thr_norm in hdi_thresholds.items():
                cs = ax.contour(
                    X_grid,
                    Y_grid,
                    Z_normalized_masked,
                    levels=[thr_norm],
                    colors="k" if level == hdi_prob else "0.5",
                    linewidths=0.7 if level == hdi_prob else 0.45,
                    linestyles="--" if level == hdi_prob else "-",
                    zorder=9,
                )
                cs.set_clip_path(clip_patch)

    # Optionally overlay scatter plot to show raw sample locations
    if show_samples:
        ax.scatter(x_samples, y_samples, c='black', s=1, alpha=0.3, zorder=10)

    # Plot true solution if provided
    if mt6_true is not None:
        T_med, N_med, P_med, E_med = MT6_TNPE(mt6_true.reshape(6, 1))
        T_plunge_med = np.degrees(np.arcsin(np.abs(T_med[2, 0])))
        B_plunge_med = np.degrees(np.arcsin(np.abs(N_med[2, 0])))
        P_plunge_med = np.degrees(np.arcsin(np.abs(P_med[2, 0])))
        x_med, y_med = kave(T_plunge_med, B_plunge_med, P_plunge_med)
        ax.scatter(x_med, y_med, facecolors='none', s=80, marker='*',
                  edgecolors='k', linewidths=0.8, zorder=30, label='True')
        
    # Plot median solution if provided
    if mt6_median is not None:
        T_med, N_med, P_med, E_med = MT6_TNPE(mt6_median.reshape(6, 1))
        T_plunge_med = np.degrees(np.arcsin(np.abs(T_med[2, 0])))
        B_plunge_med = np.degrees(np.arcsin(np.abs(N_med[2, 0])))
        P_plunge_med = np.degrees(np.arcsin(np.abs(P_med[2, 0])))
        x_med, y_med = kave(T_plunge_med, B_plunge_med, P_plunge_med)
        ax.scatter(x_med, y_med, facecolors='None', s=60, marker='o',
                  edgecolors='k', linewidths=0.65, zorder=20, label='Median')

    # Plot mode solution if provided
    if mt6_mode is not None:
        T_mode, N_mode, P_mode, E_mode = MT6_TNPE(mt6_mode.reshape(6, 1))
        T_plunge_mode = np.degrees(np.arcsin(np.abs(T_mode[2, 0])))
        B_plunge_mode = np.degrees(np.arcsin(np.abs(N_mode[2, 0])))
        P_plunge_mode = np.degrees(np.arcsin(np.abs(P_mode[2, 0])))
        x_mode, y_mode = kave(T_plunge_mode, B_plunge_mode, P_plunge_mode)
        ax.scatter(x_mode, y_mode, facecolors='None', s=50, marker='s',
                  edgecolors='k', linewidths=0.65, zorder=20, label='Mode')

    # Add legend if median or mode are plotted
    if mt6_median is not None or mt6_mode is not None:
        ax.legend(loc='upper right', fontsize=9, framealpha=0.9)

    ax.set_xlim(-1, 1)
    ax.set_ylim(-1, 1)
    # ax.set_title('Kaverina Plot - Posterior Density Distribution',
    ...
    plt.close(fig)
    # print(f"Kaverina plot saved to \n{output_path}")


def kaverina_hdi_mask(mt6_samples: np.ndarray, hdi_prob: float = 0.90):
    """
    Compute a boolean mask selecting samples inside the Kaverina HDI region.

    The HDI is defined via a 2D KDE on the Kaverina coordinates (x, y) and
    selecting the highest-density samples whose cumulative probability reaches
    ``hdi_prob`` (e.g. 0.90 for 90% HDI).
    """
    # Project MTs to Kaverina coordinates (same as in plot_kaverina_with_hdi)
    T_all, N_all, P_all, E_all = MT6_TNPE(mt6_samples)
    T_plunge = np.degrees(np.arcsin(np.abs(T_all[2, :])))
    B_plunge = np.degrees(np.arcsin(np.abs(N_all[2, :])))
    P_plunge = np.degrees(np.arcsin(np.abs(P_all[2, :])))
    x_samples, y_samples = kave(T_plunge, B_plunge, P_plunge)

    positions = np.vstack([x_samples, y_samples])
    kernel = gaussian_kde(positions)
    density = kernel(positions)  # shape (n_samples,)

    # Highest-density interval in 2D: sort densities, integrate in descending order
    ...
    if cumsum[-1] > 0:
        ...
        # Degenerate case – treat all samples as in-HD
        return np.ones_like(density, dtype=bool), density, density.min()

    idx_thr = np.searchsorted(cumsum, hdi_prob)
    idx_thr = min(idx_thr, density_sorted.size - 1)
    thr = density_sorted[idx_thr]

    mask = density >= thr
    return mask, density, thr


def plot_hudson_with_hdi(mt6_samples: np.ndarray, hdi_prob: float = 0.90,
                          min_threshold: float = 0.05,
                          output_path: str = "hudson_hdi.png",
                          grid_size: int = 300,
                          n_contour_levels: int = 20,
                          cmap: str = 'Greys',
                          show_samples: bool = False,
    ...
    """
    Plot Hudson diagram with probability density contours from MT6 posterior samples.
    ...
    mt6_samples : np.ndarray
        Moment tensor six-vectors with shape (6, n_samples).
    hdi_prob : float, default 0.90
        Probability mass for the highest density interval (e.g., 0.90 for 90% HDI).
    output_path : str
        Path to save the figure.
    grid_size : int, default 300
        Resolution of the grid for contour plotting.
    n_contour_levels : int, default 50
        Number of contour levels for smooth density visualization.
    cmap : str, default 'viridis'
        Matplotlib colormap name for density visualization.
    """
    # Project all samples onto Hudson diagram (fully vectorized)
    # Get eigenvalues for all samples at once
    T_all, N_all, P_all, E_all = MT6_TNPE(mt6_samples)

    # Convert eigenvalues to Hudson (u,v) coordinates
    # E_all has shape (3, n_samples) with sorted eigenvalues
    tau, k = E_tk(E_all)  # tau and k are (n_samples,) arrays
    u_samples, v_samples = tk_uv(tau, k)  # u and v are (n_samples,) arrays

    # Print diagnostic information
    # print(f"\n{'='*60}")
    # print(f"Hudson Plot Diagnostics:")
    # print(f"{'='*60}")
    # print(f"Number of samples: {len(u_samples)}")
    # print(f"u-coordinate: min={u_samples.min():.4f}, max={u_samples.max():.4f}, mean={u_samples.mean():.4f}, std={u_samples.std():.4f}")
    # print(f"v-coordinate: min={v_samples.min():.4f}, max={v_samples.max():.4f}, mean={v_samples.mean():.4f}, std={v_samples.std():.4f}")

    # Check if samples have sufficient variance for KDE
    ...
    # Draw Hudson diagram axes
    hudson.draw_axes(ax)

    # Only attempt KDE if there's sufficient variance
    if u_std > 1e-6 and v_std > 1e-6:
        # Create 2D KDE
        positions = np.vstack([u_samples, v_samples])
        kernel = gaussian_kde(positions)
        ...
        grid_positions = np.vstack([U_grid.ravel(), V_grid.ravel()])

        # Evaluate KDE on grid
        Z = kernel(grid_positions).reshape(U_grid.shape)

        # Build Hudson diamond polygon to mask/clip density to the valid region
        hudson_vertices = np.array([
            [0.0, 1.0],                 # top (+ISO)
            [4.0 / 3.0, 1.0 / 3.0],     # right (-CLVD)
            [0.0, -1.0],                # bottom (-ISO)
            [-4.0 / 3.0, -1.0 / 3.0],   # left (+CLVD)
        ])
        hudson_path = Path(hudson_vertices, closed=True)

        points_flat = np.column_stack([U_grid.ravel(), V_grid.ravel()])
        # Manual diamond test to avoid Path/contains_points quirks
        u_flat = points_flat[:, 0]
        v_flat = points_flat[:, 1]
        inside = (
            (v_flat <= -0.5 * u_flat + 1.0)  # top-right edge
            & (v_flat <= u_flat + 1.0)       # top-left edge
            & (v_flat >= u_flat - 1.0)       # bottom-right edge
            & (v_flat >= -0.5 * u_flat - 1.0)  # bottom-left edge
        )

        # Mask KDE outside diamond and renormalize on the valid area
        Z_masked = Z.copy()
        Z_masked.flat[~inside] = np.nan
        Z_max = np.nanmax(Z_masked)
        Z_normalized = Z_masked / Z_max

        print(f"Density range: min={np.nanmin(Z_normalized):.6f}, max={np.nanmax(Z_normalized):.6f}")

        # Clip the contour set to the Hudson diamond to prevent bleed outside
        clip_patch = plt.Polygon(hudson_vertices, closed=True,
                                 facecolor='none', edgecolor='none',
                                 transform=ax.transData)
        ax.add_patch(clip_patch)

        # ------------------------------------------------------------------
        # Compute HDI thresholds from KDE inside the Hudson diamond
        # ------------------------------------------------------------------
        z_valid = Z_masked[np.isfinite(Z_masked)].ravel()
        z_sorted = np.sort(z_valid)[::-1]  # Descending order
        z_cumsum = np.cumsum(z_sorted)

        if z_cumsum[-1] > 0:
            z_cumsum /= z_cumsum[-1]

            # We always compute 50% and the requested HDI probability
            hdi_levels = [0.50]
            if hdi_prob not in hdi_levels:
                hdi_levels.append(hdi_prob)

            hdi_thresholds: dict[float, float] = {}

            # print(f"\nHDI Threshold Values (Hudson):")
            for level in sorted(hdi_levels):
                if level <= 0 or level >= 1:
                    continue
                threshold_idx = np.searchsorted(z_cumsum, level)
                if threshold_idx >= len(z_sorted):
                    continue
                threshold = z_sorted[threshold_idx]
                threshold_normalized = threshold / Z_max
                if threshold > 0:
                    hdi_thresholds[level] = threshold_normalized
                    # print(
                    #     f"  {int(level*100)}% HDI: "
                    #     f"threshold={threshold:.6e} (normalized={threshold_normalized:.6f})"
                    # )
            ...
            for level, thr_norm in hdi_thresholds.items():
                cs = ax.contour(
                    ...
                    levels=[thr_norm],
                    colors="k" if level == hdi_prob else "0.5",
                    linewidths=0.6 if level == hdi_prob else 0.2,
                    linestyles="-" if level == hdi_prob else "--",
                    alpha=0.4,
                    zorder=9,
                )
        ...
        print(f"Warning: Insufficient variance in Hudson projection (u_std={u_std:.3e}, v_std={v_std:.3e})")

    # Optionally overlay scatter plot to show raw sample locations
    if show_samples:
        ax.scatter(u_samples, v_samples, c='black', s=1, alpha=0.3, zorder=10)
        
    # Plot true solution if provided
    if mt6_true is not None:
        T_med, N_med, P_med, E_med = MT6_TNPE(mt6_true.reshape(6, 1))
        tau_med, k_med = E_tk(E_med)
        u_med, v_med = tk_uv(tau_med, k_med)
        ax.scatter(u_med, v_med, facecolors='none', s=80, marker='*',
                  edgecolors='k', linewidths=0.8, zorder=30, label='True')
    ...
    if mt6_median is not None:
        ...
        tau_med, k_med = E_tk(E_med)
        u_med, v_med = tk_uv(tau_med, k_med)
        ax.scatter(u_med, v_med, facecolors='white', s=60, marker='o',
                  edgecolors='k', linewidths=0.8, zorder=20, label='Median')

    # Plot mode solution if provided
    if mt6_mode is not None:
        T_mode, N_mode, P_mode, E_mode = MT6_TNPE(mt6_mode.reshape(6, 1))
        tau_mode, k_mode = E_tk(E_mode)
        u_mode, v_mode = tk_uv(tau_mode, k_mode)
        ax.scatter(u_mode, v_mode,facecolors='white', s=50, marker='s',
                  edgecolors='k', linewidths=0.8, zorder=20, label='Mode')

    # Add legend if median or mode are plotted
    if mt6_median is not None or mt6_mode is not None:
        ax.legend(loc='upper right', fontsize=9, framealpha=0.9)

    ax.set_title('Hudson – Posterior 90% HDI', fontsize=12)

    fig.savefig(output_path, dpi=200, bbox_inches='tight')
    plt.close(fig)
    # print(f"Hudson plot saved to \n{output_path}")
